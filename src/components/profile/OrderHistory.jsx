    ...
    <div>
      ...
      <div className="space-y-4">
        {Array.isArray(orders) && orders.map((order) => (
          <div key={order.id || order._id || JSON.stringify(order)} className="bg-gray-700/50 rounded-lg p-4 border border-gray-600">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
              <div>
                ...
                <p className="text-white font-semibold">{formatPrice(Number(order.total) || 0)}</p>
              </div>
              <div>
                <p className="text-gray-400 text-sm">Productos</p>
                <p className="text-white">{Array.isArray(order.items) ? order.items.length : 0} items</p>
              </div>
              <div>
                <p className="text-gray-400 text-sm">Método de pago</p>
                <p className="text-white capitalize">{order.metodoPago || order.paymentMethod || 'No especificado'}</p>
              </div>
            </div>
