...
export const chileRegions = [
  ...
  },
  {
    ...
    comunas: [
      ...
      'San Miguel', 'San Ramón', 'Vitacura', 'Puente Alto',
      'Pirque', 'San José de Maipo', 'Colina', 'Lampa',
      'Tiltil', 'San Bernardo', 'Buin', 'Calera de Tango',
  ...
  },
  {
    ...
    comunas: [
      ...
      'Cauquenes', 'Chanco', 'Pelluhue', 'Curicó', 'Hualañé', 'Licantén',
      'Molina', 'Rauco', 'Romeral', 'Sagrada Familia', 'Teno', 'Vichuquén',
  ...
  },
  {
    ...
    comunas: [
      ...
      'Pinto', 'Quillón', 'San Ignacio', 'Yungay', 'Cobquecura',
      'Coelemu', 'Ninhue', 'Portezuelo', 'Quirihue', 'Ránquil',
      'Treguaco', 'San Carlos', 'Coihueco', 'San Nicolás', 'Ñiquén',
      'San Fabián'
    ]
  },
  {
    ...
    comunas: [
      ...
      'Talcahuano', 'Tomé', 'Los Ángeles', 'Antuco', 'Cabrero',
      'Laja', 'Mulchén', 'Nacimiento', 'Negrete', 'Quilaco',
      'Quilleco', 'San Rosendo', 'Santa Bárbara', 'Tucapel', 'Yumbel',
      'Alto Biobío', 'Lebú', 'Arauco', 'Cañete', 'Contulmo',
      'Curanilahue', 'Los Álamos', 'Tirúa'
    ]
  },
  {
    id: 'araucania',
    ...
    comunas: [
      ...
      'Villarrica', 'Cholchol', 'Angol', 'Collipulli', 'Curacautín',
  ...
  },
  {
    ...
    comunas: [
      'Coihaique', 'Lago Verde', 'Aysén', 'Cisnes', 'Guaitecas',
...
export const getComunasByRegion = (regionId) => {
  const region = chileRegions.find(reg => reg.id === regionId);
  return region ? region.comunas : [];
};

// Función para obtener todas las comunas (para búsquedas)
export const getAllComunas = () => {
  return chileRegions.flatMap(region => region.comunas);
};

// Función para validar si una comuna pertenece a una región
export const isValidComunaForRegion = (regionId, comuna) => {
  const comunas = getComunasByRegion(regionId);
  return comunas.includes(comuna);
};

// Función para obtener la región por comuna/ciudad
export const getRegionByCity = (cityName) => {
  return chileRegions.find(region => 
    region.comunas.some(comuna => comuna.toLowerCase() === cityName.toLowerCase())
  );
};

// Función para validar si una ciudad existe en cualquier región
export const isCityValid = (cityName) => {
  return getAllComunas().some(comuna => 
    comuna.toLowerCase() === cityName.toLowerCase()
  );
};

// Función para buscar ciudades que coincidan con un texto
export const searchCities = (searchText) => {
  return getAllComunas().filter(comuna => 
    comuna.toLowerCase().includes(searchText.toLowerCase())
  );
};